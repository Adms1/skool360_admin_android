...
public class DateUtils {
    ...
    public static String toApproximateTime(@NonNull final Context context, final long time) {
        ...
        if (diff < 50 * DateUtils.MINUTE_MILLIS)
            return String.valueOf(Math.round(diff / (double) DateUtils.MINUTE_MILLIS) + " " + context.getString(R.string.time_ago_minutes));
        if (diff < 90 * DateUtils.MINUTE_MILLIS) return context.getString(R.string.time_ago_hour);
        if (diff < 24 * DateUtils.HOUR_MILLIS)
            return String.valueOf(Math.round(diff / (double) DateUtils.HOUR_MILLIS) + " " + context.getString(R.string.time_ago_hours));
